...
function switchLoadingAnimation(){
    if(isLoading){
        isLoading = false;
        $("#camera-trigger").removeClass("loading");
        $("#trigger-text").html("Identifizieren");
    }
    else{
        isLoading = true;
        $("#camera-trigger").addClass("loading");
        $("#trigger-text").html("Warte auf Antwort");
    }
}
